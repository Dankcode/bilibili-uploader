...
// Function to retry the workflow in case of failure
async function retryWorkflow(retries = 4, delay = 600000) { // delay of 10 minutes (600,000 ms)
  let attempts = 0;

  while (attempts < retries) {
    try {
      console.log(`Starting workflow attempt ${attempts + 1}...`);
      await executeWorkflow();
      console.log('Workflow completed successfully.');
      return; // Exit if successful
    } catch (error) {
      attempts++;
      console.error(`Error in workflow attempt ${attempts}:`, error);
      ...
      const uploadId = await getValidUpload('1fb726490c0947e9967a285846af19f5');
      if (uploadId) {
        await UpdateError(uploadId, `Error in workflow attempt ${attempts}: ${error.message}`);
      }

      console.log(`Retrying workflow in 10 minutes...`);
...
const executeWorkflow = async () => {
  const apiKey= '1fb726490c0947e9967a285846af19f5';
  ...
  const databaseId = router.query.apiKey;
  try {
    ...
    if (uploadId) {
      try {
        ...
        const uploadedYoutubeUrl = await UploadVideo(videoPath, English_Name, English_Desc, videoTags, databaseId);
...
retryWorkflow(); // This will start the process with up to 4 retries and 10 minutes delay
