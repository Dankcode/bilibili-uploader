...
export function publicYouTubeAuthorization(authorization) {
  ...
  const { credentialRef: _credentialRef, ...publicFields } = authorization;
  return { ...publicFields, credentialConfigured: Boolean(authorization.credentialRef) };
}
  ...
  try {
    db.prepare(`
      INSERT INTO youtube_authorizations (
        id, google_subject, email_address, channel_id, channel_title, credential_ref,
...
export function updateYouTubeAuthorization(id, patch = {}) {
  ...
  db.prepare(`
    ...
    SET google_subject = ?, email_address = ?, channel_id = ?, channel_title = ?,
        credential_ref = ?, status = ?, enabled = ?, last_error = ?, updated_at = ?
    WHERE id = ?
  `).run(
